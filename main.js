  ...
  fetch(url)
    ...
    .then(data => {
      ...
      countryInfo.innerHTML = `
        ...
        <p>Population: ${Math.round(country['population']/10000)/100}</p>
        <p class="flag-p">Flag: <img class="flag" src="${country['flag']}"></p>
      `;
    })
}
...
body.addEventListener('mousemove', (e) => {
  guessedCountries.forEach(country => {
    if(guessedCountries.includes(e.target) || e.target == countryInfo) {
      
      if(!(e.target == countryInfo) && e.target != curCountry) {
        getCountry(`https://restcountries.eu/rest/v2/name/${e.target.getAttribute('engname')}`);
        curCountry = e.target;
      }
      
      
      // if(countryInfo.classList.contains('inactive')) { //niekoneicznie
      ...
      if(y - 26 < countryInfo.offsetHeight) {
        countryInfo.classList.add('second-version')
        countryInfo.style.left = x - countryInfo.offsetWidth/2 + 'px';
        countryInfo.style.top = y + 26 +'px';
      } else {
        countryInfo.classList.remove('second-version')
        countryInfo.style.left = x - countryInfo.offsetWidth/2 + 'px';
        countryInfo.style.top = y - countryInfo.offsetHeight - 26 +'px';
      }
    } else {
